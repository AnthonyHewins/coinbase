...
integration-test-riskier: ## Run riskier integration tests. Requires that you fill out tests/key.yaml (see tests/key.template.yaml). These hit prod (there is no sandbox), and will create orders that will basically never fill (buying BTC for $0.01)
	INTEGRATION=1 RISKY=1 go test ./tests/...
